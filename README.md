...
4. Now, open up the repository in the code editor or IDE of your choice. Open the file `config.ini` and set your OpenAI API key under the variable `KEY`.
5. Now open the file `main.py` in the `src` folder and modify the variables `experiment_participant_name`, the `chat_id`, as well as the `chat_user_id`. The documentation for the variables can be found in the code.
